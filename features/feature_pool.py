...
def build_feature_pool_from_curve(freq: List[float], amp: List[float]) -> Dict[str, float]:
    ...
    features['X35'] = float(np.var(amp_diffs)) if len(amp_diffs) > 0 else 0.0

    if len(amp_arr) > 4:
        corr = np.correlate(amp_arr - np.mean(amp_arr), amp_arr - np.mean(amp_arr), mode="full")
        corr = corr[corr.size // 2:]
        features['X36'] = float(np.max(corr[1:]) / (corr[0] + 1e-12)) if len(corr) > 1 else 0.0
    else:
        features['X36'] = 0.0

    if n > 3:
        coef = np.polyfit(freq_arr, amp_arr, 1)
        fit = coef[0] * freq_arr + coef[1]
        features['X37'] = float(np.std(amp_arr - fit))
    else:
        features['X37'] = 0.0
...
def read_curve_csv(path: Path) -> Tuple[List[float], List[float]]:
    ...
    Supports both 2-column (freq, amp) and 3-column
    (freq_injected_hz, peak_freq_meas_hz, peak_amp_dbm) formats.
    ...
    if rows and rows[0] and not _is_numeric(rows[0][0]):
        ...
        rows = rows[1:]
        if "freq_injected_hz" in header:
            freq_index = header.index("freq_injected_hz")
        if "peak_amp_dbm" in header:
            amp_index = header.index("peak_amp_dbm")
        elif "amplitude_db" in header or "amplitude_dbm" in header:
            amp_index = header.index("amplitude_db") if "amplitude_db" in header else header.index("amplitude_dbm")
    
    freq_list = []
    amp_list = []
    for row in rows:
        if len(row) <= max(freq_index, amp_index):
            continue
        try:
            freq_list.append(float(row[freq_index]))
            amp_list.append(float(row[amp_index]))
