...
MODULE_FEATURES_BY_GROUP = {
    'amp_group': ['X1', 'X2', 'X5', 'X6', 'X7', 'X10', 'X11', 'X12', 'X13', 'X19', 'X20', 'X21', 'X22', 'X36', 'X37'],
    'freq_group': ['X4', 'X8', 'X9', 'X14', 'X15', 'X16', 'X17', 'X18'],
    'ref_group': ['X1', 'X3', 'X5', 'X10', 'X11', 'X12', 'X13', 'X35'],
...
def validate_router() -> List[str]:
    """Validate router configuration consistency.

    Returns a list of issues discovered (empty if OK).
    """
    issues: List[str] = []

    # 电源模块只能在 other_group
    for group, modules in MODULE_GROUPS.items():
        if group != "other_group" and "电源模块" in modules:
            issues.append(f"电源模块出现在 {group}，应仅在 other_group")

    # fault_type 映射唯一
    seen = {}
    for key, group in FAULT_TYPE_TO_MODULE_GROUP.items():
        if key in seen and seen[key] != group:
            issues.append(f"fault_type {key} 映射到多个 group: {seen[key]} / {group}")
        seen[key] = group

    # 模块组特征必须覆盖系统分流特征
    for fault_key, sys_feats in SYSTEM_BRANCH_FEATURES.items():
        group = FAULT_TYPE_TO_MODULE_GROUP.get(fault_key)
        if not group:
            issues.append(f"fault_type {fault_key} 未映射 module_group")
            continue
        group_feats = set(MODULE_FEATURES_BY_GROUP.get(group, []))
        missing = [feat for feat in sys_feats if feat not in group_feats]
        if missing:
            issues.append(f"{fault_key} 对应 {group} 缺失特征: {missing}")
