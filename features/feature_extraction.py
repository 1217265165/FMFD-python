...
def extract_system_features(response_curve, baseline_curve=None, envelope=None) -> dict:
    ...
    if baseline_curve is not None:
        ...
        if baseline.shape == arr.shape and len(arr) > 10:
            ...
            try:
                ...
                lag_idx = peak_idx - (len(arr) - 1)  # Actual lag in bins
                if 0 < peak_idx < len(corr) - 1:
                    ...
                    frac = 0.0
                x16 = float(lag_idx + frac)  # Sub-bin lag
            except Exception:
                x16 = 0.0
            ...
            for scale in [0.995, 0.998, 0.999, 0.9995, 1.0, 1.0005, 1.001, 1.002, 1.005]:
                for shift in [-0.005, -0.002, -0.001, -0.0005, 0.0, 0.0005, 0.001, 0.002, 0.005]:
                    try:
    ...
    # X35-X37: 形态特征（差分方差 + 周期性 + 线性拟合残差）
    if len(arr) > 2:
        diff = np.diff(arr)
        x35 = float(np.var(diff))
    else:
        x35 = 0.0

    if residual.size > 4:
        try:
            corr = np.correlate(residual - np.mean(residual), residual - np.mean(residual), mode="full")
            corr = corr[corr.size // 2:]
            if corr.size > 1:
                x36 = float(np.max(corr[1:]) / (corr[0] + 1e-12))
            else:
                x36 = 0.0
        except Exception:
            x36 = 0.0
    ...
    if residual.size > 3:
        try:
            ...
            coef = np.polyfit(idx, residual, 1)
            fit = coef[0] * idx + coef[1]
            x37 = float(np.std(residual - fit))
        except Exception:
            x37 = 0.0
    else:
        x37 = 0.0
