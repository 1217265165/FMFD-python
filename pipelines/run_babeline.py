...
def _compute_real_normal_stats(traces: np.ndarray, rrs: np.ndarray) -> dict:
...
def main():
    ...
    real_stats_path = _resolve(repo_root, "Output/real_normal_stats.json")
    real_stats = _compute_real_normal_stats(aligned_traces, rrs)
    with open(real_stats_path, "w", encoding="utf-8") as f:
        json.dump(real_stats, f, ensure_ascii=False, indent=2)
    print(f"Real normal stats saved: {real_stats_path}")
    ...
    # Robust normal statistics for simulation constraints
    ...
    q_low = np.quantile(aligned_traces, 0.025, axis=0)
    q_high = np.quantile(aligned_traces, 0.975, axis=0)
    cap_normal_db = 0.4
