...
"""
# python pipelines/compare_methods.py --plot_only
...
def prepare_dataset(
    ...
    if leak_columns:
        print(
            f"[WARN] Detected {len(leak_columns)} leakage columns (auto-removed): {leak_columns[:10]}{'...' if len(leak_columns) > 10 else ''}")
...
def main():
    ...
    if args.load_params:
        params_path = Path(args.load_params) if Path(
            args.load_params).is_absolute() else PROJECT_ROOT / args.load_params
